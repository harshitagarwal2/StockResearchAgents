...
from stock_research_agents.research_conformance import validate_research_dossier as audit_research_dossier
...
def test_complete_submission_parses_for_each_supported_identity(symbol: str) -> None:
    submission = complete_research_submission(symbol)

    parsed = parse_company_research_submission_v1(submission)
...
def test_dossier_roundtrip_preserves_canonical_payload(symbol: str) -> None:
    dossier = parse_research_dossier(complete_research_submission(symbol)["dossier"])

    reparsed = ResearchDossierV1.from_json(serialize_research_dossier(dossier))
...
def test_complete_submission_passes_semantic_research_audit(symbol: str) -> None:
    dossier = complete_research_submission(symbol)["dossier"]
...
def test_document_unavailable_at_cutoff_is_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="available by cutoff"):
        parse_company_research_submission_v1(submission)


def test_filing_after_cutoff_is_rejected() -> None:
    submission = complete_research_submission("META")
...
def test_tampered_calculation_result_fails_deterministic_recomputation() -> None:
    dossier = complete_research_submission("META")["dossier"]
...
def test_research_delta_links_to_current_artifacts_and_prior_digest() -> None:
    dossier = parse_research_dossier(complete_research_submission("ORCL")["dossier"])
...
def test_entitlement_gap_is_retained_without_licensed_extract_or_false_completeness() -> None:
    dossier = parse_research_dossier(complete_research_submission("QQQ")["dossier"])
...
def test_completed_dossier_is_projected_losslessly_into_run_view() -> None:
    ...
    dossier = payload["company_research"]["dossier"]  # type: ignore[index]
...
def test_cross_symbol_submissions_do_not_share_identity_or_research_ids() -> None:
    submissions = [complete_research_submission(symbol) for symbol in ("ORCL", "META", "QQQ")]
    dossiers = [parse_company_research_submission_v1(item).dossier for item in submissions]
...
def test_fund_identity_can_report_transcripts_as_not_applicable_by_omission() -> None:
    dossier = parse_company_research_submission_v1(complete_research_submission("QQQ")).dossier
