...
from stock_research_agents.schema_bundle import (
...
def _json_submission() -> dict[str, Any]:
    return json.loads(json.dumps(complete_analytics_submission()))
...
def test_analytics_parser_rejects_restricted_transcript_segments_beyond_schema_expressiveness() -> None:
    payload = _json_submission()
    documents = payload["company_research"]["dossier"]["documents"]
...
def test_analytics_bundle_is_self_contained_and_analytics_collections_are_typed() -> None:
    bundle = load_company_analytics_submission_v1_schema_bundle()
    assert bundle["$id"] == "https://stock-research-agents.local/schemas/company-analytics-submission.v1.json"
...
def test_source_lineage_component_is_typed_and_included_in_self_contained_analytics_submission() -> None:
