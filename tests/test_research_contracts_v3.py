...
from tradingagents_portable.research_contracts import (
    COMPANY_RESEARCH_V2_PATH,
    HOST_SUBMISSION_V3_SCHEMA_PATH,
    HostSubmissionV3,
...
def valid_submission() -> dict[str, object]:
    ...
    dossier = {
        ...
        "evaluation": {
            ...
            "rubric_version": "company-research.v2",
            "checks": [
                {
                    "id": "check.evidence",
                    "status": "pass",
...
def test_v3_submission_strict_round_trip_and_schema() -> None:
    jsonschema = pytest.importorskip("jsonschema")
    payload = valid_submission()
    schema = json.loads(HOST_SUBMISSION_V3_SCHEMA_PATH.read_text())
    ...
    assert isinstance(submission, HostSubmissionV3)
...
def _v3_schema_errors(payload: object) -> list[object]:
    jsonschema = pytest.importorskip("jsonschema")
    schema = json.loads(HOST_SUBMISSION_V3_SCHEMA_PATH.read_text())
...
def test_v3_rejects_blocked_redistributable_entitlement_in_parser_and_schema() -> None:
    payload = valid_submission()
    ...
    assert _v3_schema_errors(payload)
    with pytest.raises(ValueError, match="blocked sources cannot be redistributable"):
        parse_host_submission_v3(payload)


def test_v3_rejects_non_redistributable_document_extract_in_parser_and_schema() -> None:
    payload = valid_submission()
    entitlement = payload["dossier"]["documents"][1]["entitlement"]  # type: ignore[index]
...
def test_v3_rejects_segment_extracts_from_restricted_transcript(access: str) -> None:
    payload = valid_submission()
    ...
    assert not _v3_schema_errors(payload)
    with pytest.raises(ValueError, match="cannot include segment extracts.*non-redistributable"):
        parse_host_submission_v3(payload)


def test_v3_retains_restricted_transcript_provenance_without_body_text() -> None:
    payload = valid_submission()
    transcript_document = payload["dossier"]["documents"][1]  # type: ignore[index]
    transcript_document["entitlement"].update(  # type: ignore[union-attr]
    ...
    parsed = parse_host_submission_v3(payload)
...
def test_v3_rejects_forbidden_material_recursively(field: str, message: str) -> None:
    payload = valid_submission()
...
def test_v3_rejects_credential_shaped_uri_parameters(uri: str) -> None:
    payload = valid_submission()
...
def test_v3_rejects_operations_after_dossier_completion(field: str) -> None:
    payload = valid_submission()
...
def test_v3_rejects_broken_argument_and_evidence_references() -> None:
    payload = valid_submission()
...
def test_completed_v3_requires_core_research_records(field: str) -> None:
    payload = valid_submission()
...
def test_v3_requires_completion_after_request_start() -> None:
    payload = valid_submission()
...
def test_v3_rejects_point_in_time_leakage(mutate: object, message: str) -> None:
...
def test_company_research_v2_manifest_is_ordered_and_completed_only() -> None:
    manifest = json.loads(COMPANY_RESEARCH_V2_PATH.read_text())
    stages = manifest["stages"]
    assert [stage["ordinal"] for stage in stages] == list(range(1, len(stages) + 1))
    assert stages[-1]["id"] == "publish.dossier"
    assert manifest["terminal_artifact_kind"] == "research_dossier.v3"
    assert manifest["contracts"]["terminal"].endswith("#/$defs/hostSubmission")
