...
Portable state resolves in this order:

1. `STOCKRESEARCHAGENTS_STATE_DIR`
2. compatibility fallback `TRADINGAGENTS_PORTABLE_STATE_DIR`
3. `$XDG_STATE_HOME/tradingagents-portable`
4. `~/.local/state/tradingagents-portable`
...
`analytics-init` / `create_company_analytics_run` creates a durable 26-stage run. Shared lifecycle controls accept one first-incomplete stage commit at a time, retain safe receipts and events, and resume from that boundary. Finalization strictly validates v4, rebinds report descriptors to the durable `run_id`, and atomically publishes authoritative `RunResult.artifacts`. The quality outcome index uses a hidden stage/publish sequence and is reconstructed from completed artifacts when recovery requires it; it is not part of a distributed transaction. `analytics-import` / `import_company_analytics` remains an atomic stateless path for an already-complete v4 wrapper.

## Recovery

- `host-pause` stops at a stage boundary.
- `host-resume` continues from the first incomplete stage.
- `run-cancel` requests cooperative cancellation.
- `run-cancel-ack` records host acknowledgement after in-flight work stops.
- `host-finalize` retries recoverable publication staging without weakening validation.

The host owns hard interruption and must replay an interrupted in-flight stage.
...
the default `auto` mode, Portable ensures one shared loopback viewer daemon for the durable state directory and
...
The absolute URL is runtime-local discovery metadata and is not embedded in the immutable `RunResult`. The viewer
...
digest so an incompatible daemon is retired instead of reused. The daemon rereads durable completed bundles and
...
STOCKRESEARCHAGENTS_PRESENTATION_MODE=path_only \
  uv run stock-research-agents analytics-import --input completed-v4.json
```
...
The foreground commands remain available for diagnostics and compatibility:
...
```bash
uv run stock-research-agents dashboard --host 127.0.0.1 --port 8765
```
...
| Resume repeats one stage | Portable resumes at stage boundaries | Let the host replay that stage idempotently |
...
| Analytics resume repeats one stage | Resume occurs at stage boundaries | Replay the interrupted host-owned stage idempotently, then commit it |
