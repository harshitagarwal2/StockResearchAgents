...
- **Canonical for:** reproducible live/failure evidence for provider capability claims.
...
typed receipt. “Pass” means the observed status matches the expected status;
...
| Licensed market data | prices and indicators through an entitled host `SourcePort` | validation-passing batch or truthful unavailable/denied result | entitlement and license-receipt IDs, redistribution state, no extract unless permitted |
| Social source | approved host OAuth route | validation-passing bounded batch or truthful denied result | host approval/entitlement receipt, access state, and bounded observation metadata |
...
release-evidence location selected by the release owner.
...
controls. Offline validation is necessary but never substitutes for the live
evidence.
