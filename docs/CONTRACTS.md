...
The `research-data-tools.v1` manifest is authoritative for the implemented SourceBatch v1 capability matrix. It records seven default public tools on the isolated server launched by `stock-research-data-mcp`: three SEC tools, two GDELT tools, World Bank macro, and Polymarket Gamma public search/read-only `prediction_markets` metadata. Gamma exposes no wallet, CLOB, or order endpoint; its probabilities are current market-implied observations, not truth, forecasts, or executable signals, and current search cannot reconstruct a historical snapshot. The manifest also records host-gated prices/indicators/Reddit and denied-unregistered StockTwits; Yahoo Finance/`yfinance` remains host-owned and terms-compatible, approved OAuth is required for Reddit, and FRED and Alpha Vantage are not defaults. `tradingagents-research-data` remains the manifest compatibility key. None of these tools is registered on the coordination MCP. The separate `legacy-transition.v1` manifest remains authoritative for the blocked executor-removal gates; prediction-market metadata does not close the market-data/social gaps or make removal eligible.
