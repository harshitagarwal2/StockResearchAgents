...
| Multi-source portfolio policy | Implemented as an additive host-configured terminal tool | `research_data_collect_source_portfolio` is exposed only when a host injects at least two explicit routes. It returns the full `SourcePortfolioReceipt`, preserves separate provider batches and entitlements, and receipts failures and exact dedup decisions without changing the seven default `SourceBatch` tools. The viewer audits supplied breadth and gaps but never fetches or invents sources. See [Source portfolio](SOURCE_PORTFOLIO.md). |
...
| Deterministic test proof | Supported | Test submissions span ORCL, META, QQQ, and ACME; the generated ORCL result/events/view/preview are byte-for-byte CI checked and visibly fixture-labeled. This proves generic contract and product-surface handling, not live research quality. |
| Live provider/data retrieval | Caller-owned; scheduled canary is non-gating | A bounded twice-weekly canary retains sanitized status/shape evidence for the credential-free public routes. Local tests and the canary do not certify provider credentials, arbitrary-symbol coverage, or availability/freshness for a production run. |
