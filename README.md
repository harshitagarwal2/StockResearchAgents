...
[![Python 3.11–3.14](https://img.shields.io/badge/python-3.11%20%7C%203.12%20%7C%203.13%20%7C%203.14-3776AB.svg?logo=python&logoColor=white)](pyproject.toml)
...
Python 3.11 through 3.14 and [`uv`](https://docs.astral.sh/uv/) are supported. From a source checkout:
...
[![Fixture Research Dossier Viewer preview for the deterministic ORCL demonstration](examples/generated/orcl-fixture/preview.svg)](examples/generated/orcl-fixture/preview.svg)

The checked-in preview and its JSON artifacts are visibly fixture-labeled, non-executable, and regenerated byte-for-byte in CI. See [Examples](examples/README.md#generated-product-demonstration); they are product-surface proof, not current ORCL research.
...
StockResearchAgents therefore has partial live public-source coverage, not complete live company research. Missing, stale, conflicting, or entitlement-blocked evidence remains visible. A scheduled, bounded, non-gating canary records sanitized public-provider status and contract shape, but it does not certify freshness or availability for a production run. See the [source portfolio](docs/SOURCE_PORTFOLIO.md), [research-data MCP](docs/RESEARCH_DATA_MCP.md), and [proof ledger](docs/FEATURE_PARITY.md).
...
- Follow the [community code of conduct](CODE_OF_CONDUCT.md) in issues, reviews, and project discussions.
- See the evidence-focused [roadmap](ROADMAP.md) for priorities and explicit non-goals.
