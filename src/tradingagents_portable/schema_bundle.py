"""Self-contained JSON Schema bundles for portable host integrations.
...
from tradingagents_portable.company_analytics_v1.source_lineage import SourceLineageCrosswalkV1
from tradingagents_portable.research_contracts import StrictModel

_WORKFLOW_DIRECTORY = Path(__file__).resolve().parent / "workflow"
_V4_SCHEMA_ID = "https://tradingagents-portable.local/schemas/host-submission.v4.json"
_COMPONENT_FILES = {
    "host-submission.v3.schema.json": "host_submission_v3",
    ...
    root.update(
        {
            ...
            "$id": "https://tradingagents-portable.local/schemas/analytics-bundle.v1.json",
...
def build_source_lineage_crosswalk_v1_schema() -> dict[str, Any]:
    ...
    root.update(
        {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "$id": "https://tradingagents-portable.local/schemas/source-lineage-crosswalk.v1.json",
...
def _rewrite_references(value: object, *, document_key: str) -> object:
    ...
    for key, nested in value.items():
        ...
        if key == "$ref" and isinstance(nested, str):
            ...
            if target is None:
                raise ValueError(f"v4 schema contains an unsupported external reference: {nested}")
...
def load_host_submission_v4_schema_bundle() -> dict[str, Any]:
    """Load one transportable v4 schema whose references are all root-local."""
    root = _read_schema("host-submission.v4.schema.json")
    ...
    bundle = _rewrite_references(root, document_key="host_submission_v4")
    if not isinstance(bundle, dict):  # pragma: no cover - root is a checked-in object
        raise TypeError("host-submission.v4 schema must be an object")
    bundle["$id"] = _V4_SCHEMA_ID
    existing_definitions = bundle.setdefault("$defs", {})
    if not isinstance(existing_definitions, dict):
        raise ValueError("host-submission.v4 $defs must be an object")
