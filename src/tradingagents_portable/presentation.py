...
_VIEWER_PROTOCOL_VERSION = "viewer-protocol.v2"
_VIEWER_REGISTRY_SCHEMA = "viewer-daemon-registry.v2"
...
class PresentationLink:
    """Portable receipt for a completed run's presentation location."""
...
def _fetch_json(
    ...
    *,
    ...
    if access_token is not None:
        headers["X-TradingAgents-Viewer-Token"] = access_token
...
def _child_environment(state_dir: Path) -> dict[str, str]:
    ...
    environment: dict[str, str] = {
        ...
        "TRADINGAGENTS_PORTABLE_STATE_DIR": str(state_dir),
    }
...
def _idle_ttl_seconds() -> float:
    raw = os.environ.get("STOCKRESEARCHAGENTS_PRESENTATION_IDLE_TTL_SECONDS") or os.environ.get(
        "TRADINGAGENTS_PORTABLE_PRESENTATION_IDLE_TTL_SECONDS"
    )
    if raw is None:
    ...
    try:
        ...
        return subprocess.Popen(  # noqa: S603 - fixed interpreter/module and validated local arguments
            [
                sys.executable,
                "-m",
                "tradingagents_portable.viewer_daemon",
                "--state-dir",
                str(state_dir),
                "--registry",
                str(registry_path),
...
def ensure_completed_run_presentation(
    ...
    *,
    ...
    if mode is not None:
        ...
        selected_mode = (
            (
                os.environ.get("STOCKRESEARCHAGENTS_PRESENTATION_MODE")
                or os.environ.get("TRADINGAGENTS_PORTABLE_PRESENTATION_MODE")
                or "auto"
            )
            .strip()
            .lower()
