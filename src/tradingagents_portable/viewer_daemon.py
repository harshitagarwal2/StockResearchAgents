...
class _DurablePublicationCoordinator:
    ...
    def control(self, run_id: str) -> dict[str, Any]:
        ...
        company = CompanyResearchCoordinator(
            lifecycle_store,
            result_store,
            memory_store_factory=memory_store_factory,
            profile=company_lifecycle.COMPANY_RESEARCH_COORDINATOR.profile,
        )
        analytics = CompanyResearchCoordinator(
            lifecycle_store,
            result_store,
            memory_store_factory=memory_store_factory,
            profile=company_lifecycle.COMPANY_ANALYTICS_COORDINATOR.profile,
        )
        host = HostRunCoordinator(lifecycle_store, result_store, memory_store_factory=memory_store_factory)
        selected = select_run_coordinator(run_id, company, host, (analytics,))
        try:
            return selected.control(run_id)
        finally:
            if selected.memory_store is not None:
                selected.memory_store.close()
...
def _serve(args: argparse.Namespace, state_dir: Path, registry_path: Path) -> int:
    ...
    server = create_dashboard_server(
        ...
        session_cookie_name=f"tradingagents_viewer_{args.state_fingerprint[:16]}",
    )
