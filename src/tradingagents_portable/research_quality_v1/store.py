...
def _default_quality_dir() -> Path:
    ...
    if xdg_state_home:
        return Path(xdg_state_home).expanduser() / "tradingagents-portable" / "quality"
    return Path.home() / ".local" / "state" / "tradingagents-portable" / "quality"
...
class QualityStore:
    ...
    def __init__(self, state_dir: str | os.PathLike[str] | None = None) -> None:
        ...
        self._legacy_forecast_ids: dict[str, str | None] = {}
        self._loaded = False
        ...
        if self._state_dir is not None:
            ...
            if registration_dir.exists():
                for path in sorted(registration_dir.glob("*.json")):
                    ...
                    forecasts = tuple(self._load_forecast(item, receipt.run_id) for item in raw["forecasts"])
            ...
            if outcome_dir.exists():
                for forecast_dir in sorted(path for path in outcome_dir.iterdir() if path.is_dir()):
                    forecast_id = self._legacy_forecast_ids.get(forecast_dir.name, forecast_dir.name)
    ...
    def _load_forecast(self, value: object, run_id: str) -> Forecast:
        """Read current records and migrate pre-namespace local development records in memory."""
        try:
            return Forecast.from_dict(value)
        except ValueError as exc:
            if "globally namespaced by run_id" not in str(exc) or not isinstance(value, dict):
                raise
            legacy_id = value.get("forecast_id")
            if not isinstance(legacy_id, str) or value.get("run_id") != run_id:
                raise
            migrated_id = f"{run_id}.{legacy_id}"
            forecast = Forecast.from_dict(value | {"forecast_id": migrated_id})
            existing = self._legacy_forecast_ids.get(legacy_id)
            if existing is not None and existing != migrated_id:
                self._legacy_forecast_ids[legacy_id] = None
            elif legacy_id not in self._legacy_forecast_ids:
                self._legacy_forecast_ids[legacy_id] = migrated_id
            return forecast
