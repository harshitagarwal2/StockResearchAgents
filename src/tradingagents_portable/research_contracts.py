...
The portable boundary accepts bounded facts, extracts, references, and deterministic
...
RESEARCH_SCHEMA_VERSION = "2026-08-03.v3"
COMPANY_RESEARCH_WORKFLOW_ID = "tradingagents.company-research.v2"
WORKFLOW_DIRECTORY = Path(__file__).resolve().parent / "workflow"
COMPANY_RESEARCH_V2_PATH = WORKFLOW_DIRECTORY / "company-research.v2.json"
HOST_SUBMISSION_V3_SCHEMA_PATH = WORKFLOW_DIRECTORY / "host-submission.v3.schema.json"
...
class CoverageDimension(StrictModel):
    ...
    entitlement_policy: Literal["public_only", "host_entitled_allowed"]
...
class CompanyResearchRequest(StrictModel):
    schema_version: Literal["2026-08-03.v3"]
    request_id: str
    requested_at: str
    cutoff_at: str
...
@dataclass(frozen=True, slots=True)
class ResearchDossierV3(StrictModel):
    schema_version: Literal["2026-08-03.v3"]
    dossier_id: str
    ...
    @classmethod
    def from_json(cls, value: str) -> ResearchDossierV3:
        try:
...
ResearchDossier = ResearchDossierV3


@dataclass(frozen=True, slots=True)
class HostSubmissionV3(StrictModel):
    schema_version: Literal["2026-08-03.v3"]
    workflow_id: Literal["tradingagents.company-research.v2"]
    request: CompanyResearchRequest
    dossier: ResearchDossierV3
...
def parse_research_dossier(value: object) -> ResearchDossierV3:
    """Parse and fully validate one completed ``host-submission.v3`` dossier."""
    return ResearchDossierV3.from_dict(value)


def serialize_research_dossier(value: ResearchDossierV3) -> str:
    """Serialize a validated dossier using stable, bounded canonical JSON."""
    if not isinstance(value, ResearchDossierV3):
        raise TypeError("value must be ResearchDossierV3")
    return value.to_json()


def validate_research_dossier(dossier: ResearchDossierV3, cutoff: str) -> None:
    """Re-run whole-dossier invariants against an explicit host cutoff."""
    if not isinstance(dossier, ResearchDossierV3):
        raise TypeError("dossier must be ResearchDossierV3")
    if dossier.as_of_at != cutoff:
        raise ValueError("dossier.as_of_at must exactly match the requested cutoff")
    ResearchDossierV3.from_dict(dossier.to_dict())


def parse_host_submission_v3(payload: object) -> HostSubmissionV3:
    """Parse one strict request-plus-terminal-dossier host submission."""
    if isinstance(payload, Mapping):
        ...
        if isinstance(request, Mapping) and isinstance(dossier, Mapping):
            ...
            if (
                ...
                raise ValueError("dossier.completed_at cannot precede request.requested_at")
    return HostSubmissionV3.from_dict(payload)
