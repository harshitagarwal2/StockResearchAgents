...
class RunStore:
    def __init__(
        ...
        *,
        ...
        self._staged: dict[str, tuple[RunResult, tuple[RunEvent, ...]]] = {}
    ...
    def _bundle_json(self, result: RunResult, events: tuple[RunEvent, ...]) -> str:
        return json.dumps(
            {
    ...
    def put(self, result: RunResult, events: tuple[RunEvent, ...]) -> None:
        with self._lock:
            ...
            self._validate_events(run_id, frozen_events)
            state_dir = self._state_path()
            if state_dir is not None:
                self._publish_durable_bundle(state_dir, result, frozen_events, write_intent=True)
            self._results[run_id] = result
            self._events[run_id] = frozen_events
            self._current_run_id = run_id
        ...
        with self._lock:
            ...
            if state_dir is not None:
                _atomic_write(
                    ...
                    self._bundle_json(result, frozen_events),
                )
            self._staged[run_id] = (result, frozen_events)
    ...
    def put_events(self, run_id: str, events: Iterable[RunEvent]) -> None:
        ...
        with self._lock:
            ...
            safe_run_id = _validate_run_id(run_id)
            frozen_events = tuple(events)
            self._validate_events(safe_run_id, frozen_events)
            state_dir = self._state_path()
            if state_dir is not None:
                _atomic_write(state_dir / "events" / f"{safe_run_id}.json", serialize_run_events(frozen_events))
                self._write_current(state_dir, safe_run_id)
            self._events[safe_run_id] = frozen_events
    ...
    def append_event(self, event: RunEvent) -> None:
        ...
        with self._lock:
            ...
            events = (*self._events.get(run_id, ()), event)
            self._validate_events(run_id, events)
            state_dir = self._state_path()
            if state_dir is not None:
                _atomic_write(state_dir / "events" / f"{run_id}.json", serialize_run_events(events))
