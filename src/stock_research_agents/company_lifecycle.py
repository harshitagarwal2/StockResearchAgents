"""Durable caller-driven lifecycle for ``company-analytics.v1``.
...
tool inputs/results remain outside the StockResearchAgents core.
"""
...
class RunControlV1(StrictModel):
    """Strict caller-facing projection of one durable lifecycle checkpoint."""
    ...
    execution_mode: str
    execution_mode_readiness: str
    execution_mode_locally_ready: bool
    status: str
    storage_status: str
...
def _rebind_submission_run_id(value: object, previous_run_id: str, run_id: str) -> object:
    if isinstance(value, dict):
        return {key: _rebind_submission_run_id(item, previous_run_id, run_id) for key, item in value.items()}
    if isinstance(value, list):
        return [_rebind_submission_run_id(item, previous_run_id, run_id) for item in value]
...
def publication_lifecycle_run_id(events: Sequence[RunEvent]) -> str | None:
    """Return the durable lifecycle that produced a canonical publication.

    Direct imports deliberately carry no origin marker. Lifecycle publications
    retain the association in the terminal canonical event so detached readers
    can enforce publication readiness using only durable state.
    """
    for event in reversed(events):
        if event.kind != EventKind.RUN or event.status != "completed":
            continue
        origin = event.data.get(_PUBLICATION_ORIGIN_KEY)
        if origin is None:
            return None
        if not isinstance(origin, Mapping) or origin.get("kind") != _LIFECYCLE_PUBLICATION_KIND:
            raise ValueError("canonical publication event has an invalid publication origin")
        lifecycle_run_id = origin.get("lifecycle_run_id")
        if not is_lifecycle_run_id(lifecycle_run_id):
            raise ValueError("canonical publication event has an invalid lifecycle run ID")
        return str(lifecycle_run_id)
    return None
...
def require_completed_publication(
    ...
    A direct completed import has no lifecycle origin and remains readable.
    Lifecycle-produced publications are keyed by the durable origin retained in
    their terminal canonical event, so both identifiers share one readiness
    decision.
    """
    resolved = run_id
    if run_id == "current":
        current_run_id = getattr(result_store, "current_run_id", None)
        resolved = (current_run_id() or run_id) if callable(current_run_id) else run_id
    events = result_store.get_events(resolved)
    lifecycle_run_id = publication_lifecycle_run_id(events or ())
    if events is not None and lifecycle_run_id is None:
        return resolved
    if coordinator is not None and is_lifecycle_run_id(resolved):
        try:
            alias_control = coordinator.control(resolved)
        except KeyError:
            alias_control = None
        if alias_control is not None and (
            alias_control["status"] != "completed" or alias_control["publication_pending"]
        ):
            raise ValueError(f"run publication is not complete: {resolved}")
        result_run_id = alias_control.get("result_run_id") if alias_control is not None else None
        if isinstance(result_run_id, str):
            resolved = result_run_id
            lifecycle_run_id = publication_lifecycle_run_id(result_store.get_events(resolved) or ())
    if lifecycle_run_id is None:
        return resolved
    if coordinator is None:
        raise ValueError(f"run publication origin is unavailable: {resolved}")
    try:
        control = coordinator.control(lifecycle_run_id)
    except KeyError:
        raise ValueError(f"run publication origin is unavailable: {resolved}") from None
    if control["status"] != "completed" or control["publication_pending"] or control.get("result_run_id") != resolved:
        raise ValueError(f"run publication is not complete: {resolved}")
    return resolved
...
def _stage(record: Mapping[str, Any], stage_id: str) -> Mapping[str, Any]:
    try:
        ...
        raise ValueError(f"unknown company analytics stage: {stage_id}") from exc
...
class CompanyAnalyticsCoordinator:
    ...
    def create(
        ...
        *,
        ...
        selected_pack = plan.get("research_pack")
        if not isinstance(selected_pack, Mapping) or not isinstance(selected_pack.get("pack_id"), str):
            raise ValueError("lifecycle profile must select a research pack")
        ...
        record: dict[str, Any] = {
            ...
            "system_boundary": deepcopy(plan["system_boundary"]),
        ...
        _emit(
            ...
            kind=EventKind.RUN,
            status=LifecycleStatus.PREPARED.value,
            message=f"Durable {self.profile.workflow_profile} lifecycle prepared for caller-owned execution.",
            data={
                "workflow_profile": self.profile.workflow_profile,
    ...
    def control(self, run_id: str) -> dict[str, Any]:
        ...
        return RunControlV1(
            ...
            run_id=run_id,
            research_pack_id=str(record["research_pack_id"]),
            decision_memory_enabled=bool(record["decision_memory_enabled"]),
            execution_mode=str(record["execution_mode"]),
            execution_mode_readiness=str(record["execution_mode_readiness"]),
            execution_mode_locally_ready=bool(record["execution_mode_locally_ready"]),
            status=(LifecycleStatus.FINALIZING.value if publication_pending else str(record["status"])),
            storage_status=str(record["status"]),
            publication_pending=publication_pending,
            result_ready=result_ready,
            memory_ready=memory_ready,
            sidecars_ready=sidecars_ready,
            revision=int(record["revision"]),
            next_stage_id=str(next_stage["id"]) if next_stage is not None else None,
            completed_stage_ids=[str(item) for item in record["completed_stage_ids"]],
            active_stage_id=str(record["active_stage_id"]) if record["active_stage_id"] is not None else None,
            active_attempt=int(record["active_attempt"]) if record["active_attempt"] is not None else None,
            cancel_requested=record["status"] == LifecycleStatus.CANCEL_REQUESTED.value,
            cancel_reason=str(record["cancel_reason"]) if record["cancel_reason"] is not None else None,
            result_run_id=str(record["result_run_id"]) if record["result_run_id"] is not None else None,
            checkpoint={
                "request_hash": _checkpoint_hash(record, "request"),
                "topology_hash": _checkpoint_hash(record, "topology"),
    ...
    def start(self, run_id: str, expected_revision: int) -> dict[str, Any]:
        def mutate(record: dict[str, Any]) -> None:
            ...
            _emit(record, kind=EventKind.RUN, status="running", message="Caller executor started company analytics.")
    ...
    def next_stage(self, run_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "context": {
                ...
                "system_boundary": deepcopy(record["system_boundary"]),
    ...
    def _validate_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
        ...
        for key in ("receipt_id", "stage_id", "capability_id", "execution_call_id"):
    ...
    def commit_stage(
        ...
        *,
        ...
        if stage_id == self.profile.terminal_stage_id:
            ...
            selected_mode = current.get("execution_mode") or "sequential"
            ...
            bound_submission: Any = parsed.to_dict()
            bound_stages = bound_submission["run_card"]["stages"]
            for stage_receipt in bound_stages:
                stage_receipt["output_digest"] = committed_digests[stage_receipt["stage_id"]]
            bound_submission["run_card"]["coordinator_commitments"] = [item.to_dict() for item in commitments]
            bound_submission["quality_receipt"]["stage_digests"] = [
                {"stage_id": stage_receipt["stage_id"], "sha256": stage_receipt["output_digest"]}
                for stage_receipt in bound_stages
            ]
            previous_run_id = parsed.run_card.run_id
            rebound_run_id = analytics_run_id(bound_submission)
            bound_submission = _rebind_submission_run_id(bound_submission, previous_run_id, rebound_run_id)
            parsed = self.profile.parse_terminal(bound_submission)
        ...
        def mutate(record: dict[str, Any]) -> None:
            ...
            execution_completion_attested = bool(starts and matching_completions)
            ...
            _emit(
                ...
                message=(
                    ...
                    if final_submission is not None
                    else f"Opaque output envelope for {stage_id} committed; caller-owned content remains unverified."
                ),
                stage_id=stage_id,
                data={
                    "ordinal": stage["ordinal"],
                    ...
                    "execution_completion_attested": execution_completion_attested,
                    # StockResearchAgents receives bounded execution receipts; it does not
                    # observe the caller's private agent/tool execution itself.
    ...
    def resume(self, run_id: str, expected_revision: int) -> dict[str, Any]:
        def mutate(record: dict[str, Any]) -> None:
            ...
            _emit(
                ...
                kind=EventKind.RUN,
                status="resumed",
                message="Caller executor resumed at the first incomplete stage boundary.",
            )
    ...
    def acknowledge_cancel(self, run_id: str, expected_revision: int, execution_receipt_id: str) -> dict[str, Any]:
        if _SAFE_ID_PATTERN.fullmatch(execution_receipt_id) is None:
            raise ValueError("execution_receipt_id must be a safe, non-empty identifier")
        ...
        def mutate(record: dict[str, Any]) -> None:
            ...
            record["cancel_ack_receipt_id"] = execution_receipt_id
            record["active_stage_id"] = None
            record["active_attempt"] = None
            _emit(
                record,
                kind=EventKind.RUN,
                status="cancelled",
                message="Caller acknowledged cooperative cancellation.",
            )
    ...
    def _build_final_result(self, record: Mapping[str, Any]) -> tuple[CompanyAnalyticsResultV1, tuple[RunEvent, ...]]:
        ...
        if not isinstance(record.get("finalization_completed_at"), str):
            raise ValueError("finalization_completed_at must be recorded before publication")
        if not isinstance(imported, CompanyAnalyticsResultV1):
            raise TypeError("company analytics profile returned an unexpected result type")
        result = imported
        events = tuple(
            replace(
                event,
                data={
                    **event.data,
                    _PUBLICATION_ORIGIN_KEY: {
                        "kind": _LIFECYCLE_PUBLICATION_KIND,
                        "lifecycle_run_id": str(record["run_id"]),
                    },
                },
            )
            if event.kind == EventKind.RUN and event.status == "completed"
            else event
            for event in draft.events
        )
        return result, events

    def finalize(self, run_id: str, expected_revision: int) -> tuple[CompanyAnalyticsResultV1, tuple[RunEvent, ...]]:
        record = self._get(run_id)
