...
class SourceObservation:
    ...
    canonical normalized source record. Callers must select the scope explicitly
    so the digest can never be interpreted through an implicit fallback.
    """
    ...
    def from_dict(cls, value: object) -> SourceObservation:
        ...
        if "content_sha256_scope" not in raw:
            raise ValueError("source observation content_sha256_scope is required")
        payload = dict(raw)
...
class SourceBatch:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.query, _SOURCE_QUERY_CLASSES):
            raise TypeError("SourceBatch.query must use a stock_research_agents_host query contract")
...
def validate_source_response(capability: str, query: SourceQuery, batch: SourceBatch) -> SourceBatch:
    ...
    if not isinstance(query, _SOURCE_QUERY_CLASSES):
        raise TypeError("source queries must use a stock_research_agents_host query contract")
